...
def visualize_cameras(colored_camera_dicts, sphere_radius, camera_size=0.1, geometry_file=None, geometry_type='mesh'):
    ...
    for color, camera_dict in colored_camera_dicts:
        ...
        for img_name in camera_dict:
            K = np.array(img_name['K']).reshape((4, 4))
            C2W = np.array(img_name['transform_matrix']).reshape((4, 4))
            if idx == 1:
                C2W[:3, :3] = np.array(((0, -1, 0), (1, 0, 0), (0, 0, 1))) @ np.array(((1, 0, 0), (0, 0, 1), (0, -1, 0))) @ C2W[:3, :3]
                C2W[2, 3] = C2W[2, 3] + 0.4
            C2W[2, 2] = C2W[2, 2] + 0.1
            img_size = img_name['img_size']
...
if __name__ == '__main__':
    ...
    sphere_radius = 1.
    train_cam_dict = json.load(open(os.path.join(base_dir, '/media/huayanling/d9020a15-be70-448c-a6c3-9400b7f1a855/data/kitti_360/static_scene/generate/generate_poses.json')))
    test_cam_dict = json.load(open(os.path.join(base_dir, '/media/huayanling/d9020a15-be70-448c-a6c3-9400b7f1a855/data/kitti_360/static_scene/generate/generate_poses.json')))
    path_cam_dict = json.load(open(os.path.join(base_dir, '/media/huayanling/d9020a15-be70-448c-a6c3-9400b7f1a855/data/kitti_360/static_scene/generate/generate_poses.json')))
    camera_size = 0.1
