...
import time
from data_loader_split_nuscenes import load_data_split_nuscenes
import numpy as np
...
def ddp_train_nerf_auto_labeling(rank, args):
    ...
    ray_samplers = load_data_split_nuscenes(args.datadir, args.scene, split='train',
                                   try_load_min_depth=args.load_min_depth)
    val_ray_samplers = load_data_split_nuscenes(args.datadir, args.scene, split='validation',
...
def config_parser():
    ...
    # dataset options
    parser.add_argument("--datadir", type=str, default='/media/linger/ubuntu/null_max/data_all/tanks_and_temples/tanks_and_temples', help='input data directory')
    parser.add_argument("--scene", type=str, default='tat_intermediate_Playground', help='scene name')
    parser.add_argument("--testskip", type=int, default=8,
                        help='will load 1/N images from test/val sets, useful for large datasets like deepvoxels')
    # model size
    parser.add_argument("--netdepth", type=int, default=2, help='layers in coarse network')
    parser.add_argument("--netwidth", type=int, default=4, help='channels per layer in coarse network')
...
def train():
    ...
    torch.multiprocessing.spawn(ddp_train_nerf_auto_labeling,
                                args=(args,),
