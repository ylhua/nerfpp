...
def weights_init_density(m):
    if isinstance(m, nn.Linear):
        nn.init.xavier_uniform_(m.weight.data)
        if m.bias is not None:
            # nn.init.zeros_(m.bias.data)
            nn.init.constant_(m.bias.data, -5.0)

class MLPNet(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_viewdirs=3,
                 skips=[4], use_viewdirs=False, num_semantic_classes=47):
        '''
        ...
        for i in range(D):
            self.base_layers.append(
                nn.Sequential(nn.Linear(dim, W), nn.Softplus())
            )
            dim = W
        ...
        self.sigma_layers.apply(weights_init_density)      # xavier init
        ...
        for i in range(1):
            ...
            semantic_layers.append(nn.Softplus())
            dim_ = W // 2
