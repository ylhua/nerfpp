...
def ddp_train_nerf(rank, args):
    ...
    for global_step in range(start + 1, start + 1 + args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            # transient_maks = ray_batch['transient_mask'].to(rank)
            ...
            if 'autoexpo' in ret:
                ...
                loss = rgb_loss + args.lambda_autoexpo * (
                            torch.abs(scale - 1.) + torch.abs(shift)) + semantic_loss * args.semantic_wgt
                depth_loss = img2mse(ret['depth_fgbg'] * ray_batch['depth_isVal'], depth_gt * ray_batch['depth_isVal'])
            else:
                # rgb_loss = img2mse(ret['rgb'] * ray_batch['transient_mask'], rgb_gt * ray_batch['transient_mask'])
                rgb_loss = img2mse(ret['rgb'], rgb_gt)
                semantic_loss = CrossEntropyLoss(ret['semantic'], semantic_gt.long())
                depth_loss = img2mse(ret['depth_fgbg'] * ray_batch['depth_isVal'], depth_gt * ray_batch['depth_isVal'])
                loss = rgb_loss + semantic_loss * args.semantic_wgt + 0.00004 * depth_loss
            ...
            scalars_to_log['lebel_{}/depth_loss'.format(m)] = depth_loss.item()
        ...
        if global_step % args.i_img == 0 or global_step == start + 1:
            ...
            if rank == 0:  # only main process should do this
                ...
                log_view_to_tb(writer, global_step, log_data, gt_img=val_ray_samplers[idx].get_img(),
                               gt_semantic=val_ray_samplers[idx].get_semantic(),
                               gt_depth=val_ray_samplers[idx].get_depth_gt(), mask=None, prefix='val/')
            ...
            if rank == 0:  # only main process should do this
                ...
                log_view_to_tb(writer, global_step, log_data, gt_img=ray_samplers[idx].get_img(),
                               gt_semantic=ray_samplers[idx].get_semantic(), gt_depth=ray_samplers[idx].get_depth_gt(),
                               mask=None, prefix='train/')
